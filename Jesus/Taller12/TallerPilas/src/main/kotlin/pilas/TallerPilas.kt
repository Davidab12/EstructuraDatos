...
fun contarVerbos(palabras: IStack<String>): Int {
    val secondaryStack = palabras.copy()
    val stackPeekAndPop = { stack:IStack<*> -> val stackElement = stack.peek(); stack.pop(); stackElement.toString() }
    val wordIsVerb = { word:String -> (word.endsWith("ar") || word.endsWith("er") || word.endsWith("ir")) }

    var counter = 0
    while(!secondaryStack.isEmpty) {
        var word = stackPeekAndPop(secondaryStack)
        if(wordIsVerb(word))
            counter++
    }
...
fun sumarParesTresCifras(pila: IStack<Int>): Int {
    ...
    val stackPeekAndPop = { stack:IStack<*> -> val stackElement = stack.peek(); stack.pop(); stackElement.toString().toInt() }
