...
class Empleado {
    ...
    fun darFechaNacimiento(): String {
        return "${this.fechaNacimiento.darDia()} del mes ${this.fechaNacimiento.darMes()} de ${this.fechaNacimiento.darAño()}"
    }
    ...
    fun darFechaIngreso(): String {
        return "${this.fechaIngreso.darDia()} del mes ${this.fechaIngreso.darMes()} de ${this.fechaIngreso.darAño()}"
    }
    ...
    fun calcularEdad(): Int {
        ...
        val edad: Period = Period.between(
            LocalDate.of(
                this.fechaNacimiento.darAño(),
                this.fechaNacimiento.darMes(),
                this.fechaNacimiento.darDia()
            ), LocalDate.now()
        )
        return edad.years
    }
    ...
    fun calcularAntiguedad(): Int {
        val hoy = darFechaActual()
        val antiguedad: Period = Period.between(
            LocalDate.of(
                this.fechaIngreso.darAño(),
                this.fechaIngreso.darMes(),
                this.fechaIngreso.darDia()
            ), LocalDate.now()
        )
    ...
    fun calcularPrestaciones(): Double {
        return if (this.calcularAntiguedad() > 0) ((this.calcularAntiguedad() * salario) / 12) else 0.0
    }
     ...
     */
    fun tipo(): String {
        return when (this.calcularAntiguedad()) {
            0 -> "freshman"
            in 1..3 -> "sophomore"
            in 3..9 -> "junior"
            else -> "senior"
    ...
    fun bonificacion(numPersonasACargo: Int): Double {
        return this.salario * 0.07 * numPersonasACargo
    }
}